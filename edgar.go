...
Takes a timestamp, and concurrency(channel size) as arguments. Returns a channel
...
*/
func GetCIKList(ts time.Time, concurrency int) (chan *CIKEntry, error) {
	client := &http.Client{}
	chl := make(chan *CIKEntry, concurrency)
	defer close(chl) //close channel on return
	...
	if err == nil && resp.StatusCode == 200 {
		...
		parseCIKList(resp.Body, chl)
	}
	return chl, nil
}

func parseCIKList(r io.Reader, chl chan *CIKEntry) {
	scanner := bufio.NewScanner(r)
	for l := 0; scanner.Scan(); l++ {
		s := strings.Split(scanner.Text(), ":")
		glog.Infof("CIK list line %d: %s, %s", l, s[0], s[1])
		chl <- &CIKEntry{s[1], s[0], l}
	}
}
